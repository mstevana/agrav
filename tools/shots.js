...
for (const track of TRACK_IDS.filter(t => !ONLY || ONLY.includes(t))) {
  ...
  for (let i = 0; i < shots; i++) {
    ...
    await page.evaluate((hide) => { const sc = window.__agrav.scene(); const fx = sc.fx; for (const e of fx.live) e.obj?.parent?.remove(e.obj); fx.live.length = 0;
      ...
      for (const c of sc.crafts.values()) c.shield?.removeFromParent(); sc.three.traverse(o => { if (hide.includes(o.name)) o.visible = false; }); const p = window.__agrav.client.myPose(); const { camera, THREE } = window.__agrav;
