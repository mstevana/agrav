...
// of each engine's path, billboarded toward the camera and left in world
// space, so it stays where the craft has been, tapering and closing to a point
// as it goes.
//
// It is laid from the nozzle, so it begins at the engine with nothing between
// the two. The fire is drawn over its first few metres and bends along the
// same path (see exhaust.js), which is what makes them read as one plume
// rather than a cone with a ribbon stuck to the end of it.
// ============================================================================
...
export class EngineTrail {
  ...
  /**
   * Grow and rebuild the ribbon from the nozzle's world position. `emit` is the current thrust; at
   * zero the engine is out and the ribbon stops growing, then ages away. Oldest point first.
   */
  update(nozzle, emit, camera, now) {
    const pts = this.pts;
    if (emit > 0.05 && (!pts.length || pts[pts.length - 1].p.distanceToSquared(nozzle) > MIN_STEP * MIN_STEP)) {
      pts.push({ p: nozzle.clone(), t: now, e: emit });
  ...
  /**
   * Where this engine was `dist` metres ago, for the fire to bend along. Walks back from `head` --
   * the live nozzle, not the newest stored point, because the ribbon only grows while the engine is
   * lit and off the throttle that point falls behind. Runs out of history: the oldest point.
   */
  sampleBack(dist, head, out) {
    out.copy(head);
    let left = dist, from = head;
    for (let i = this.pts.length - 1; i >= 0; i--) {
      const p = this.pts[i].p;
      const seg = from.distanceTo(p);
      if (seg >= left) { if (seg > 1e-6) out.lerpVectors(from, p, left / seg); else out.copy(p); return out; }
      left -= seg; from = p; out.copy(p);
    }
    return out;
  }
